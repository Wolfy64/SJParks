...
const validateUserInput = require('../configurations/validator')
  .validateUserInput;
...
async function incoming(req, res) {
  ...
  if (text[2] || (text[1] && !['start', 'stop'].includes(text[0])))
    return respond(defaultResponseMessage);
    ...
    await db.User.findOne({ phone: phone }, async (err, user) => {
      ...
      if (text[1]) {
        ...
        await db.Park.findOne({ code: text[1] }, (err, park) => {
          ...
          if (!park)
            return respond(res, false, {
              msg: `${
                text[1]
              } is not a valid park code. ${defaultResponseMessage}`
            });
          // Has user subscribed in the past?
          if (user.parks.indexOf(park._id) < 0)
            return respond(res, false, {
              msg: `You never subscribed to notifications for ${park.name}.`
            });
          ...
          if (index < 0)
            return respond(res, false, {
              msg: `You have already unsubscribed from notifications for ${
                park.name
          ...
          park.save(function(err, updated) {
            ...
            respond(res, true, {
              msg: `So sad to see you go! You have unsubscribed to notifications for ${
                updated.name
              }.`
            });
          });
        ...
        if (!user.parks.length)
          return respond(res, false, {
            msg: 'You never subscribed to notifications from Parks and Rec'
          });
        ...
        await db.Park.find({ _id: { $in: user.parks } }, function(err, parks) {
          ...
          if (!parks)
            return respond(res, false, {
              msg: 'Your parks have been destroyed by irrational disaster!'
            });
          ...
          respond(res, false, {
            msg:
              'So sad to see you go! You have unsubscribed to notifications from Parks and Rec.'
          });
        });
      }
    });
    ...
    if (text[0] === 'start' && !text[1]) {
      ...
      await db.User.findOne({ phone: phone }, async (err, user) => {
        ...
        await db.Park.find({ _id: { $in: user.parks } }, (err, parks) => {
          ...
          if (!parks)
            return respond(res, false, {
              msg: 'Hmmm? You have no parks to resubscribe to.'
            });
          ...
          respond(res, false, {
            msg:
              'Thanks for resubscribing to notifications from San Jose Parks!'
          });
        });
      });
    } else {
      ...
      // Find the park
      await db.Park.findOne({ parkID: text[text.length - 1] }, async function(
        err,
        park
      ) {
        if (err)
        ...
        await db.User.findOne({ phone: phone }, function(err, user) {
          ...
          if (park.users.indexOf(user._id) >= 0)
            return respond(
              `You are already subscribed to ${park.name}.`,
              false
            );
...
async function create(req, res) {
  return new Promise(async (resolve, reject) => {
    const { errors, isValid, data } = await validateUserInput(req).catch(err =>
      console.log(err)
    );
...
async function destroy(req, res) {
  if (req.user) {
    const removedUser = await db.User.findByIdAndRemove(req.user._id).catch(
      err => {
        console.log(err);
...
function findUpdate(req, res) {
  db.User.findById(req.params.userId)
    .then(user => {
      const update = user.updates.find(
        update => update._id === req.params.updateId
      );
