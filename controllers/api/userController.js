...
function create(req, res) {
  console.log(
    `>[USER:37:26]> Creating  user with 'phone = ${req.body.phone}' ...`
  );
  const {
    isAdmin,
    userName,
...
function update(req, res) {
        ...
        bcrypt.hash(newPassword, newUser.salt, (err, hash) => {
          ...
          newUser
            ...
            .catch(err =>
              res.status(420).send({
                Success: false,
                Error: `Error saving updated user. Error thrown: ${
                  err.message
                }.`
              })
            );
  ...
  console.log(
    `>[USER:190:27]> User with '_id = ${req.params.id} has been updated'`
  );
}
...
const destroy = (req, res) => {
  console.log(
    `>[USER:197:27]> Destroying user with '_id = ${req.params.id}'...`
  );
          ...
          .catch(err =>
            res.status(467).json({
              ...
              error: `Error removing "user: ${
                user._id
              }" from "updates". Error Thrown: ${err.message}`
            })
          );
      });
  ...
  console.log(
    `>[USER:238:27]> User with '_id = ${req.params.id} has been destroyed...'`
  );
};
