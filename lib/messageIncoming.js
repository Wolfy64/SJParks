    ...
    // 'start [parkcode]' equivalent to '[parkcode]' i.e. resubscribe process is same as subscription process
      ...
      db.Park.findOne({parkID: text[text.length-1]}, function(err, park) {
        ...
        db.User.findOne({phone: phone}, function(err, user) {
          if (err) return respond('Derp! Please text back again later.');
          // New user
          if (!user) user = new db.User({phone: phone, firstName: 'Need2Update', lastName: 'Need2Update', email: 'Need2Update@adam.henry'+Math.random().toString(36).slice(2), userName: 'Need2Update'+Math.random().toString(36).slice(2), password: 'Need2Update'+Math.random().toString(36).slice(2)});
          // Already subscribed
